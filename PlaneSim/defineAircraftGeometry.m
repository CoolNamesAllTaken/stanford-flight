function geom = defineAircraftGeometry(aircraftName)
	...
	if (strcmpi(aircraftName, 'LittlePuckerCruise'))
		...
		geom = AircraftGeom('Little Pucker Cruise', mass, liftSurfaces, dragSurfaces, {motor});
	elseif (strcmpi(aircraftName, 'LittlePuckerClimb'))
		mass = 1.97; % [kg]
		...
		motor_v_inf = [0 13.06 26.11]; % [m/s] static, cruise, pitch
		motor_T = [1.204 0.239 0]; % [N] static, cruise, pitch
		motor_e_prop = [0.0048 0.00307 0]; % [kg/W] static, cruise, pitch
		motor_e_motor = 0.795; % static and cruise
		motor = Motor(motor_v_inf, motor_T, motor_e_prop, motor_e_motor);

		geom = AircraftGeom('Little Pucker Climb', mass, liftSurfaces, dragSurfaces, {motor});
	elseif (strcmpi(aircraftName, 'LittlePuckerDescend'))
		mass = 1.97; % [kg]

		wing1_b = 1.27;
		wing1_S = 0.18;
		wing1_C_Lmin = 0.3; % takeoff condition; wing C_Lmax
		wing1_C_D0 = 0.02; % zero lift C_D
		wing1_e = 0.7; % rectangular wing efficiency factor
		wing1 = LiftSurface(wing1_b, wing1_S, wing1_C_Lmin, wing1_C_D0, wing1_e);
		wing2 = LiftSurface(wing1_b, wing1_S, wing1_C_Lmin, wing1_C_D0, wing1_e);
		liftSurfaces = {wing1, wing2};
		...
		geom = AircraftGeom('Little Pucker Descend', mass, liftSurfaces, dragSurfaces, {motor});
	end
end